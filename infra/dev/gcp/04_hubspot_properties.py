...
HubSpot is the central lead-profile source AND the SP-2 orchestrator; these
properties carry the parts of the 9-pointer profile that don't map to standard
contact fields, the SP-2 lead-context (research semantics written by the email
service), plus the pipeline metadata (stage, probability, engagement counters).
...
# SP-2 stages: voice (Vapi) replaces text_voice; scheduling stage retired.
STAGES = ["ingested", "profiled", "email_outreach", "voice_outreach",
          "meeting_scheduled", "unresolved"]
...
PROPERTIES = [
    ...
    # --- SP-2 lead context (research semantics; written by the email push,
    #     NOT touched by the gateway/trigger so routing is unaffected) --------
    {"name": "lqabr_lead_context", "label": "LQABR Lead Context",
    ...
    # --- engagement counters (incremented via the MCP write path) ----------
