...
DUPLICATE OF ``packages/lqabr_core/lqabr_core/gcp_id_token.py`` -- deliberately.
``agents/gateway/requirements.txt`` states the gateway must not depend on
lqabr_core, so the two copies exist to honour that boundary rather than break
it for forty lines. Keep them in sync; there are no other copies.
