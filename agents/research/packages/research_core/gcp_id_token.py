...
LOCAL COPY -- deliberate. ``research_core`` is standalone by design and
``tests/test_standalone.py`` fails the build if it imports ``lqabr_core``. The
same file also lives in ``packages/lqabr_core/lqabr_core/gcp_id_token.py`` and
``agents/gateway/lib/soloai/id_token.py``; keep all four in sync.
